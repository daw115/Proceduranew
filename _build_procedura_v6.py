...
PROC_SUBNAV = ('<div class="stickerbar"><b>Opis procesu — skok do:</b>'
    + ''.join(mini_sticker(h,t) for h,t in [
        ('#proc-pse','Skróty i pliki PSE'),('#proc-a','IDCC(a)'),
        ('#proc-bcd','IDCC(b)–(d)'),('#proc-mail','Szablony maili')])
    + '</div>')

proces_body = (proces_body
    + '<h3 style="margin-top:1.8em">Opis procesu — kroki istotne dla PSE</h3>'
    + '<p class="lead">Na podstawie <i>Core IDCC Flow-Based Business Process Documentation v5.0</i> (02-03-2026). '
      'Tylko kroki, w których PSE działa lub jest adresatem; nazwy pod-procesów pozostawione po angielsku, '
      'IDCC(b)–(d) opisane wspólnie (różnice w tabeli). Kody plików w notacji FIDx.</p>'
    + PROC_SUBNAV
    + '<div class="proc">'
    + ''.join(load_frag(f) for f in ['process2_glos.html','process2_a.html',
              'process2_bcd.html','process2_mail.html'])
    + '</div>')
...
{section('sec-proces','Proces IDCC — kroki istotne dla PSE (BPD)','1',proces_body,[('#sec-katalog','katalog plików'),('#sec-legenda','statusy')])}
...
# ── 8. Wkład do stickerów: STICKERY_IDCC.md (to samo źródło co tablica kart) ──
DOCNAME = OUT.name
# mini-stickery per sekcja — wyciągnięte z wygenerowanego dokumentu (stickerbar w każdej sekcji)
sec_minis = {}
for sid, bar in re.findall(r'<section id="(sec-[^"]+)".*?<div class="stickerbar">(.*?)</div>', doc, re.S):
    sec_minis[sid] = re.findall(r'<a class="mini" href="([^"]+)">▸\s*([^<]+)</a>', bar)

lines = ['# Stickery IDCC — wkład (karty skrócone z hiperłączami)','',
         f'Hiperłącza prowadzą do kotwic dokumentu `{DOCNAME}`. '
         'Format: tytuł, skrót 1–3 zdania, linki.','']
for href, ic, title, desc in STICKERS:
    lines.append(f'## {ic} {title}')
    lines.append('')
    lines.append(desc)
    lines.append('')
    lines.append(f'- **Pełna instrukcja:** [{DOCNAME}{href}]({DOCNAME}{href})')
    for mh, mt in sec_minis.get(href.lstrip('#'), []):
        lines.append(f'- Powiązane — {mt}: [{DOCNAME}{mh}]({DOCNAME}{mh})')
    lines.append('')
ST = ROOT / 'STICKERY_IDCC.md'
ST.write_text('\n'.join(lines), encoding='utf-8')
print('Zapisano:', ST, f'({len(STICKERS)} kart)')
